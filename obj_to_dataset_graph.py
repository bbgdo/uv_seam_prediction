import sys
import warnings
from pathlib import Path

import numpy as np
import torch
from torch_geometric.data import Data

# Suppress noisy trimesh warnings about missing textures / materials
warnings.filterwarnings("ignore", category=UserWarning)
import trimesh  # noqa: E402  (import after warning filter)


# Helpers
def _safe_normalize(v: np.ndarray, eps: float = 1e-8) -> np.ndarray:
    """Row-wise L2 normalisation, safe for zero-length vectors."""
    norms = np.linalg.norm(v, axis=-1, keepdims=True)
    return v / np.where(norms < eps, eps, norms)


# Core: seam detection
def _detect_seam_edges(mesh: trimesh.Trimesh) -> dict:
    """
    Return {(vi, vj): is_seam} for all edges (vi < vj).

    Boundary edges are always seams. Interior edges are seams if either
    endpoint has different UV coordinates across the two adjacent faces.
    Handles both per-merged-vertex and per-face-corner UV layouts.
    """
    faces = mesh.faces  # (F, 3)  geometric vertex indices
    has_uv = (
        hasattr(mesh, "visual")
        and hasattr(mesh.visual, "uv")
        and mesh.visual.uv is not None
        and len(mesh.visual.uv) > 0
    )

    # Build edge -> faces map  {(vi,vj): [f0, f1, ...]}
    edge_to_faces: dict[tuple, list] = {}
    for f_idx, face in enumerate(faces):
        for k in range(3):
            vi = face[k]
            vj = face[(k + 1) % 3]
            key = (min(vi, vj), max(vi, vj))
            edge_to_faces.setdefault(key, []).append(f_idx)

    seam_map: dict[tuple, bool] = {}

    if not has_uv:
        # No UV data – mark boundary edges as seams, interior as non-seam
        for edge, face_list in edge_to_faces.items():
            seam_map[edge] = (len(face_list) == 1)
        return seam_map

    uv = mesh.visual.uv  # (V, 2) indexed by geometric vertex idx
    # trimesh may give UV per face-corner instead of per merged vertex when
    # the mesh has UV splits.  We detect which case we're in:
    uv_is_per_face_corner = (len(uv) == len(faces) * 3)

    def get_uv_for_vertex_in_face(face_idx: int, geom_vertex: int) -> np.ndarray:
        """Return the UV that face `face_idx` assigns to `geom_vertex`."""
        if uv_is_per_face_corner:
            face = faces[face_idx]
            local_pos = np.where(face == geom_vertex)[0]
            if len(local_pos) == 0:
                return np.array([0.0, 0.0])
            fc_idx = face_idx * 3 + local_pos[0]
            return uv[fc_idx]
        else:
            # UV indexed by geometric vertex
            if geom_vertex < len(uv):
                return uv[geom_vertex]
            return np.array([0.0, 0.0])

    UV_EPS = 1e-5 # tolerance for UV comparison

    for edge, face_list in edge_to_faces.items():
        vi, vj = edge
        if len(face_list) == 1:
            # Boundary edge -> always a seam
            seam_map[edge] = True
        elif len(face_list) == 2:
            f0, f1 = face_list
            uv_vi_f0 = get_uv_for_vertex_in_face(f0, vi)
            uv_vi_f1 = get_uv_for_vertex_in_face(f1, vi)
            uv_vj_f0 = get_uv_for_vertex_in_face(f0, vj)
            uv_vj_f1 = get_uv_for_vertex_in_face(f1, vj)
            split_i = np.linalg.norm(uv_vi_f0 - uv_vi_f1) > UV_EPS
            split_j = np.linalg.norm(uv_vj_f0 - uv_vj_f1) > UV_EPS
            seam_map[edge] = bool(split_i or split_j)
        else:
            # Non-manifold edge – treat as seam
            seam_map[edge] = True

    return seam_map


# Core: dihedral angle computation

def _compute_dihedral_angles(
    mesh: trimesh.Trimesh,
    edges: np.ndarray, # (E, 2)  geometric vertex indices
    edge_to_faces: dict,
) -> np.ndarray:
    """
    Vectorised dihedral angle for every edge (in radians, range [0, π]).

    For boundary edges (only one adjacent face) the angle is set to π
    (flat-seam sentinel).
    """
    face_normals = mesh.face_normals # (F, 3)  unit normals

    angles = np.full(len(edges), np.pi, dtype=np.float32)

    for idx, (vi, vj) in enumerate(edges):
        key = (min(vi, vj), max(vi, vj))
        face_list = edge_to_faces.get(key, [])
        if len(face_list) >= 2:
            n0 = face_normals[face_list[0]]
            n1 = face_normals[face_list[1]]
            cos_a = np.clip(np.dot(n0, n1), -1.0, 1.0)
            angles[idx] = float(np.arccos(cos_a))

    return angles # (E,)


# Main processing function

def process_mesh(file_path: str | Path) -> Data | None:
    """
    Load an .obj file and return a PyG Data object.

    Graph schema
    ------------
    x: [N, 6] vertex coords (xyz) + vertex normals (nxnynz)
    edge_index: [2, E] undirected edges (each geometric edge appears once
                           in each direction so E = 2 * num_unique_edges)
    edge_attr: [E, 4] edge length | dihedral angle | Δnormal_vi | Δnormal_vj
    y: [E] 1.0 = seam, 0.0 = not a seam

    Returns None on failure.
    """
    file_path = Path(file_path)

    # Load mesh
    try:
        # process=False keeps raw face-vertex attributes (needed for UV)
        mesh = trimesh.load(str(file_path), process=False, force="mesh")
        if not isinstance(mesh, trimesh.Trimesh):
            print(f"  [SKIP] {file_path.name}: not a single Trimesh object.")
            return None
        if len(mesh.faces) == 0 or len(mesh.vertices) == 0:
            print(f"  [SKIP] {file_path.name}: empty mesh.")
            return None
    except Exception as exc:
        print(f"  [ERROR] {file_path.name}: {exc}")
        return None

    vertices  = np.asarray(mesh.vertices,      dtype=np.float32)   # (V, 3)
    vert_nrms = np.asarray(mesh.vertex_normals, dtype=np.float32)  # (V, 3)
    faces     = np.asarray(mesh.faces,          dtype=np.int64)    # (F, 3)

    # Seam detection
    seam_map = _detect_seam_edges(mesh)

    # Build canonical edge set + edge->face lookup for dihedral angles

    unique_edges_set: set[tuple] = set()
    edge_to_faces: dict[tuple, list] = {}

    for f_idx, face in enumerate(faces):
        for k in range(3):
            vi = int(face[k])
            vj = int(face[(k + 1) % 3])
            key = (min(vi, vj), max(vi, vj))
            unique_edges_set.add(key)
            edge_to_faces.setdefault(key, []).append(f_idx)

    # Sort for determinism
    unique_edges = np.array(sorted(unique_edges_set), dtype=np.int64)  # (UE, 2)
    num_unique   = len(unique_edges)

    # Node features
    # x = [coords | normals]  shape (V, 6)
    x = torch.from_numpy(np.concatenate([vertices, vert_nrms], axis=1))

    # Edge features
    vi_idx = unique_edges[:, 0]
    vj_idx = unique_edges[:, 1]

    # 1. Edge length
    edge_vec    = vertices[vj_idx] - vertices[vi_idx] # (UE, 3)
    edge_length = np.linalg.norm(edge_vec, axis=1, keepdims=True).astype(np.float32)

    # 2. Dihedral angle
    dihedral = _compute_dihedral_angles(mesh, unique_edges, edge_to_faces)
    dihedral  = dihedral[:, None] # (UE, 1)

    # 3. Difference in vertex normals at endpoints
    #    ||n_vi - n_vj||  captures how much the surface bends at the edge
    delta_nrm = np.linalg.norm(
        vert_nrms[vi_idx] - vert_nrms[vj_idx], axis=1, keepdims=True
    ).astype(np.float32) # (UE, 1)

    # 4. Dot product of vertex normals (complementary curvature signal)
    dot_nrm = np.einsum(
        "ij,ij->i", _safe_normalize(vert_nrms[vi_idx]),
                    _safe_normalize(vert_nrms[vj_idx])
    ).astype(np.float32)[:, None] # (UE, 1)

    edge_attr_np = np.concatenate(
        [edge_length, dihedral, delta_nrm, dot_nrm], axis=1
    ) # (UE, 4)

    # Labels
    labels = np.array(
        [1.0 if seam_map.get((int(e[0]), int(e[1])), False) else 0.0
         for e in unique_edges],
        dtype=np.float32
    ) # (UE,)

    # Both directions per unique edge (mirror features + labels)
    src = np.concatenate([vi_idx, vj_idx])
    dst = np.concatenate([vj_idx, vi_idx])
    edge_index = torch.from_numpy(
        np.stack([src, dst], axis=0).astype(np.int64)
    ) # (2, 2*UE)

    edge_attr = torch.from_numpy(
        np.tile(edge_attr_np, (2, 1)) # mirror attrs
    ) # (2*UE, 4)

    y = torch.from_numpy(
        np.tile(labels, 2) # mirror labels
    ) # (2*UE,)

    data = Data(
        x          = x,
        edge_index = edge_index,
        edge_attr  = edge_attr,
        y          = y,
        num_nodes  = len(vertices),
    )
    data.file_path = str(file_path)

    return data


# Statistics helper

def print_stats(data: Data, file_name: str) -> None:
    num_nodes = data.num_nodes
    num_edges = data.edge_index.shape[1]          # directed (both ways)
    num_unique_edges = num_edges // 2

    seam_mask    = data.y.bool()
    num_seams    = seam_mask.sum().item()
    num_nonseams = num_edges - num_seams
    seam_pct     = 100.0 * num_seams / max(num_edges, 1)
    non_pct      = 100.0 - seam_pct

    # pos_weight for BCEWithLogitsLoss = num_negative / num_positive
    pos_weight = num_nonseams / max(num_seams, 1)

    print(f"\n{'='*60}")
    print(f"  File          : {file_name}")
    print(f"  Nodes (V)     : {num_nodes}")
    print(f"  Unique Edges  : {num_unique_edges}")
    print(f"  Directed Edges: {num_edges}  (stored both ways)")
    print(f"  Edge features : {data.edge_attr.shape[1]}  "
          f"[length | dihedral | Δnorm | dot_norm]")
    print(f"  --- Class Balance (over directed edges) ---")
    print(f"  Seam (1)  : {num_seams:>8d}  ({seam_pct:.2f} %)")
    print(f"  NonSeam(0): {num_nonseams:>8d}  ({non_pct:.2f} %)")
    print(f"  pos_weight for BCEWithLogitsLoss : {pos_weight:.4f}")
    print(f"{'='*60}")


# Main block

if __name__ == "__main__":
    import argparse

    parser = argparse.ArgumentParser(
        description="Build PyG UV-Seam dataset from .obj files."
    )
    parser.add_argument(
        "mesh_dir",
        nargs="?",
        default="./meshes",
        help="Directory containing .obj files (default: ./meshes)",
    )
    parser.add_argument(
        "--max-meshes",
        type=int,
        default=5,
        help="Number of meshes to process (default: 5)",
    )
    parser.add_argument(
        "--save",
        action="store_true",
        help="Save the dataset list as dataset.pt in the current directory.",
    )
    args = parser.parse_args()

    mesh_dir = Path(args.mesh_dir)
    if not mesh_dir.is_dir():
        print(f"[ERROR] Directory not found: {mesh_dir}")
        sys.exit(1)

    obj_files = sorted(mesh_dir.glob("**/*.obj"))
    if not obj_files:
        print(f"[ERROR] No .obj files found in {mesh_dir}")
        sys.exit(1)

    print(f"\nFound {len(obj_files)} .obj file(s) in '{mesh_dir}'.")
    print(f"Processing first {min(args.max_meshes, len(obj_files))} …\n")

    dataset: list[Data] = []
    outliers: list[str] = []  # files with 0 seam edges
    failed = 0

    for obj_file in obj_files[: args.max_meshes]:
        print(f"Processing: {obj_file.name} …", end=" ", flush=True)
        data = process_mesh(obj_file)
        if data is None:
            failed += 1
            continue
        print("OK")
        if data.y.sum().item() == 0:
            outliers.append(obj_file.name)
            print(f"  [OUTLIER] {obj_file.name}: 0 seam edges — skipped.")
            continue
        dataset.append(data)
        print_stats(data, obj_file.name)

    # Aggregate stats across all processed meshes
    if dataset:
        total_nodes  = sum(d.num_nodes          for d in dataset)
        total_edges  = sum(d.edge_index.shape[1] for d in dataset)
        total_seams  = sum(d.y.sum().item()      for d in dataset)
        total_nonseam = total_edges - total_seams
        agg_pos_weight = total_nonseam / max(total_seams, 1)

        print(f"\n{'#'*60}")
        print(f"  AGGREGATE OVER {len(dataset)} MESH(ES)")
        print(f"  Total Nodes        : {total_nodes}")
        print(f"  Total Directed Edges: {total_edges}")
        print(f"  Total Seam Edges   : {int(total_seams)}  "
              f"({100*total_seams/max(total_edges,1):.2f} %)")
        print(f"  Aggregate pos_weight: {agg_pos_weight:.4f}")
        print(f"\n  ► Use in training:")
        print(f"      pos_weight = torch.tensor([{agg_pos_weight:.4f}])")
        print(f"      criterion  = torch.nn.BCEWithLogitsLoss(pos_weight=pos_weight)")
        print(f"{'#'*60}\n")

    if args.save and dataset:
        out_path = Path("dataset.pt")
        torch.save(dataset, out_path)
        print(f"Dataset saved to {out_path.resolve()}  ({len(dataset)} graphs)")

    if outliers:
        print(f"\n{'!'*60}")
        print(f"  OUTLIERS — {len(outliers)} file(s) with 0 seam edges (excluded from dataset):")
        for name in outliers:
            print(f"    • {name}")
        print(f"{'!'*60}")

    if failed:
        print(f"\n[WARNING] {failed} file(s) failed to process.")

    print("\nDone.")


# Usage example (library mode)
#
# from pyg_uv_seam_dataset import process_mesh
#
# data = process_mesh("path/to/my_mesh.obj")
# print(data)
# # Data(x=[1024, 6], edge_index=[2, 6144], edge_attr=[6144, 4], y=[6144])
#
# # Training snippet:
# import torch
# pos_weight = torch.tensor([your_calculated_pos_weight])
# criterion = torch.nn.BCEWithLogitsLoss(pos_weight=pos_weight)
# logits = model(data.x, data.edge_index, data.edge_attr)  # (E,)
# loss = criterion(logits, data.y)